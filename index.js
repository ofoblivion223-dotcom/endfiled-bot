    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'setup') {
            ...
            try {
                ...
                await channel.send({
                    content: '─── エンドフィールド管理局 稼働開始 ───\nこのチャンネルで毎朝4時に日課を案内します。',
        ...
        if (commandName === 'test-reminder') {
            console.log(`Test-reminder command triggered by ${user.tag} in ${guild.name}`);
            await interaction.reply({ content: 'テストリマインドを送信中...', ephemeral: true });
            try {
                await scheduler.sendDailyReminders(client);
                console.log('Test-reminder execution finished in scheduler.');
                await interaction.editReply('テストリマインドの送信が完了しました！チャンネルを確認してください。');
            } catch (err) {
                console.error('Error during test-reminder execution:', err);
                await interaction.editReply('テストリマインド実行中にエラーが発生しました。Railwayのログを確認してください。');
            }
        }
    }
